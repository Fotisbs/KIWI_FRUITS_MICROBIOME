...
View(data.frame(sample_data(Fungi_KIWI_2024_Annotated)))

Fungi_KIWI_2024_An_Merged <- merge_samples(Fungi_KIWI_2024_Annotated, "Tissue_MCP_Storage")

##After merging the sample data info are lost and you have to import again the characteristics of each merged treatment##
write.table(data.frame(sample_data(Fungi_KIWI_2024_An_Merged)), file="sample_data_Fungi_KIWI_2024_An_Merged.txt", quote = F,col.names = NA, sep="\t")

SampleDataNew67 <- read.table("sample_data_Fungi_KIWI_2024_An_Merged.txt", header=T,sep = "\t",row.names = 1)

sample_data(Fungi_KIWI_2024_An_Merged) <- SampleDataNew67

View(data.frame(sample_data(Fungi_KIWI_2024_An_Merged)))

Fungi_KIWI_2024_An_Merged

##Transform to relative abundance %##
Fungi_KIWI_2024_An_Merged_100 <- transform_sample_counts(Fungi_KIWI_2024_An_Merged, function(OTU) 100*OTU/sum(OTU))

Fungi_KIWI_2024_An_Merged_100
...
##Phyloseq object##
physeq_raw <- Fungi_KIWI_2024_An_Merged_100
...
top5_idx <- order(r2_values, decreasing = TRUE)[1:5]
...
taxa_arrows$Label <- sapply(taxa_arrows$TaxonID, function(tid) {
  ...
  species <- tax_tbl_clean[tid, "Species"]
  if (!is.na(genus) && genus != "" && !is.na(species) && species != "") {
    paste0(genus, " ", species)
  } else if (!is.na(genus) && genus != "") {
    paste0(genus, " sp.")
  } else if (!is.na(tax_tbl_clean[tid, "Family"]) && tax_tbl_clean[tid, "Family"] != "") {
    paste0("Unclassified ", tax_tbl_clean[tid, "Family"])
  } else {
    "Unclassified"
  }
})
...
theme_minimal() +
  labs(title = "Fungi–Amino Acids NMDS Triplot",
...
##Save plot##
ggsave("Fungi_Amino_Acids_NMDS_Triplot.pdf", plot = p, width = 8, height = 7)



#Fungi_KIWI vs Organic_Acids##
# Load and scale metabolites##
...
theme_minimal() +
  labs(title = "Fungi–Organic Acids NMDS Triplot",
       subtitle = paste("NMDS Stress =", round(ord.nmds.bray$stress, 3)),
       fill = "Cold Storage",
       shape = "Tissue Type") +
theme(legend.position = "right",
...
# Save plot
ggsave("Fungi_Organic_Acids_NMDS_Triplot.pdf", plot = p, width = 8, height = 7)

##Fungi_KIWI vs Carbohydrates##
##Load and scale metabolites##
...
theme_minimal() +
  labs(title = "Fungi–Carbohydrates Acids NMDS Triplot",
       subtitle = paste("NMDS Stress =", round(ord.nmds.bray$stress, 3)),
       fill = "Cold Storage",
       shape = "Tissue Type") +
theme(legend.position = "right",
...
##Save plot##
ggsave("Fungi_Carbohydrates_NMDS_Triplot.pdf", plot = p, width = 8, height = 7)

##Fungi_KIWI vs Phenolics##
##Load and scale metabolites##
...
theme_minimal() +
  labs(title = "Fungi–Phenolics NMDS Triplot",
       subtitle = paste("NMDS Stress =", round(ord.nmds.bray$stress, 3)),
       fill = "Cold Storage",
       shape = "Tissue Type") +
theme(legend.position = "right",
...
##Save plot##
ggsave("Fungi_Phenolics_NMDS_Triplot.pdf", plot = p, width = 8, height = 7)