...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, 
                     trimLeft = 9, trimRight = 50, maxN=0,  
...
write.table(track, file = "readQC_Fungi.txt", col.names = NA, sep = "\t", quote = FALSE)

##Taxonomically classify the sequences##
taxa <- assignTaxonomy(seqtab.nochim, minBoot = 80, "/mnt/4abcb8af-e2eb-47e2-86d6-eed71f6d5304/00_home_guests_do_not_move/fotisbs/wshp_foldrs/dbs/UNITE_ITS_sh_general_release_s_10.05.2021/sh_general_release_dynamic_s_10.05.2021.fasta", multithread=TRUE, tryRC = TRUE)
...
samdfFungi <- read.table(file = "samdf.txt", header = TRUE, row.names = 1, sep = "\t")

View(samdfFungi)

##Construct the phyloseq object## 
Fungi_KIWI_2024 <- phyloseq(otu_table(seqtab.nochim, 
                                             taxa_are_rows=FALSE),
                                   sample_data(samdfFungi),
                                   tax_table(taxa))
...
sequences <- Biostrings::DNAStringSet(taxa_names(Fungi_KIWI_2024))

names(sequences) <- taxa_names(Fungi_KIWI_2024)

ps <- merge_phyloseq(Fungi_KIWI_2024, sequences)
...
taxa_names(Fungi_KIWI_2024) <- paste("ASV",str_pad(1:length(taxa_names(Fungi_KIWI_2024)),4, pad = "0"),sep = "")

rank_names(Fungi_KIWI_2024)

Fungi_KIWI_2024

##SAVE THE FINAL PHYLOSEQ OBJECT##
saveRDS(Fungi_KIWI_2024, file = "Fungi_KIWI_2024.RDS")

##Check the object features##
View(data.frame(otu_table(Fungi_KIWI_2024)))
View(data.frame(tax_table(Fungi_KIWI_2024)))
View(data.frame(sample_data(Fungi_KIWI_2024)))

##Create tables,and check number of features for each taxa##
table(tax_table(Fungi_KIWI_2024)[, "Kingdom"], exclude = NULL)
table(tax_table(Fungi_KIWI_2024)[, "Phylum"], exclude = NULL) 
table(tax_table(Fungi_KIWI_2024)[, "Class"], exclude = NULL) 
table(tax_table(Fungi_KIWI_2024)[, "Order"], exclude = NULL)
table(tax_table(Fungi_KIWI_2024)[, "Family"], exclude = NULL)
table(tax_table(Fungi_KIWI_2024)[, "Genus"], exclude = NULL)
table(tax_table(Fungi_KIWI_2024)[, "Species"], exclude = NULL)
...
Fungi_KIWI_2024_Clean <- subset_taxa(Fungi_KIWI_2024, Phylum %in% (c("p__Ascomycota", "p__Basidiomycota")))

##Removed not assigned (NA), uncharacterized Phylum##
Fungi_KIWI_2024_Clean <- subset_taxa(Fungi_KIWI_2024_Clean, !is.na(Phylum) & !Phylum %in% (c("", "uncharacterized")))

Fungi_KIWI_2024_Clean <- prune_taxa(taxa_sums(Fungi_KIWI_2024_Clean)>0, Fungi_KIWI_2024_Clean)

##Replace the NA names with the classified leftmost (higher level annotated) taxa##
Fungi_KIWI_2024_Annotated <- Fungi_KIWI_2024_Clean

for(i in 1:nrow(tax_table(Fungi_KIWI_2024_Annotated))){
  for(j in 2:ncol(tax_table(Fungi_KIWI_2024_Annotated))){
    if(is.na(tax_table(Fungi_KIWI_2024_Annotated)[i,j])){
      tax_table(Fungi_KIWI_2024_Annotated)[i,j] <- tax_table(Fungi_KIWI_2024_Annotated)[i,j-1]
    }
  }
}

####SAVE THE FINAL Annotated phyloseq object and continue with the statistical analysis scripts##
saveRDS(Fungi_KIWI_2024_Annotated, file = "Fungi_KIWI_2024_Annotated.RDS")