...
# Rarefy dataset
set.seed(123)
physeq_rare <- rarefy_even_depth(Bacteria_KIWI_2024_Annotated, rngseed = 123, verbose = FALSE)
...
# Merge with metadata
alpha_wide_fact <- merge(alpha_wide, data.frame(sample_data(Bacteria_KIWI_2024_Annotated)), 
                         by = "row.names")
